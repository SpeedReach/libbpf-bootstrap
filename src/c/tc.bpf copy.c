...
uint8_t server_addr[4] = { 192,168,1,1 };
...
static __always_inline int handle_udp(struct __sk_buff *ctx, struct udphdr *udphdr)
{
	...
	if (bpf_ntohs(udphdr->dest) == 7072) {
		...
		for (int i = 0; i < 2; i++) {
			...
			u32 new_addr = bpf_htons(*(u32*) server_addr);
			ret = bpf_skb_store_bytes(
				ctx, ETH_SIZE + IP_SIZE + offsetof(struct iphdr, daddr), &new_addr,
			...
			ret = bpf_clone_redirect(ctx, 20, 0);
